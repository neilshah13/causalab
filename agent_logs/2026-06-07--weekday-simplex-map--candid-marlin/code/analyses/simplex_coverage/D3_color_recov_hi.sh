#!/bin/bash
#SBATCH --job-name=D3_color_recov_hi
...
for HD in Blue Red Green; do for K in 24 32 40; do
  uv run python "$CODE/recovery_test.py" --concept colors --layer 31 --holdout $HD --carrier "A color:" --k $K --n-samples 20000 --margin 1.5 --tag "${HD}_k${K}" --prompts "$CP" --experiment-root "$CROOT"
done; done
for KEEP in "Red,Green,Blue" "Red,Orange,Yellow,Green,Blue,Purple,Pink,Brown"; do
  uv run python "$CODE/sparse_recovery.py" --concept colors --layer 31 --carrier "A color:" --keep "$KEEP" --k-list "16,24,32,40" --methods "pca,diff,cca" --n-samples 12000 --margin 2.0 --prompts "$CP" --experiment-root "$CROOT"
done
echo "D3 DONE"
